  ...
  if(!mongoose.connections[0].readyState){
   ...
   let hoods = {}
    for(let item of products){
      if(item.brand in hoods){
        if(!hoods[item.brand].color.includes(item.color) && item.availableQty > 0){
          hoods[item.brand].color.push(item.color)
        }
        if(!hoods[item.brand].size.includes(item.size) && item.availableQty > 0){
          hoods[item.brand].size.push(item.size)
        }
      }
      else{
        hoods[item.brand] = JSON.parse(JSON.stringify(item))
        if(item.availableQty > 0){
          hoods[item.brand].color = [item.color]
          hoods[item.brand].size = [item.size]
        }
      }
    }
   return {
